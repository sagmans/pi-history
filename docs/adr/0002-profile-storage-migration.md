...
The first fixed TUI initialization freezes recognized direct regular files from the former HOME-derived directory into `~/.pi/agent/pi-history-profile-migration-v1/`. Recognized files are `config.json`, `config.local.json`, `global.json`, and `project-<sha256>.json`; symlinks, unknown files, lock directories, and temporary artifacts are excluded. A completion marker freezes an empty source too, so later default-profile prompts never enter future profile imports.

Absent profile storage imports only this frozen snapshot. Atomic directory claims, private staging, no-replace file publication, and completion markers preserve existing targets, including empty directories. Migration failure emits a fixed operational notice and does not weaken profile-isolated live paths.
